...
- v0.13.9 (dep refresh — May 2026): focused upgrade pass against an aging toolchain. **Kotlin** `2.0.21` → `2.1.20` (~1.5 years of K2 compiler improvements, stdlib additions, KSP2 parity). **AGP** `8.7.3` → `8.8.0`. **KSP** `2.0.21-1.0.27` → `2.1.20-1.0.32`. **Compose BOM** `2025.01.01` → `2025.04.01` (5 months of M3 / runtime / animation fixes). **Room** `2.7.0` → `2.7.1`. **androidx.core** `1.15.0` → `1.16.0`. Kotlin serialization plugin tracked Kotlin. **Active pain points cleared:** (a) `lint { checkReleaseBuilds = false }` workaround removed — the `NonNullableMutableLiveDataDetector` crash that forced us to skip release lint on the AGP 8.7.3 / Kotlin 2.0.21 stack is gone on AGP 8.8, `lintVitalAnalyzeRelease` now runs cleanly as part of `assembleRelease`. (b) Attempted to swap the manual-entry Expense/Income toggle back from `FilterChip` to `SegmentedButton` (the right Material 3 semantic) — `SegmentedButton` still isn't shipping in BOM 2025.04.01 (must've landed in a later BOM); FilterChip preserved as-is for now. Full unit suite green, both debug + release APKs build clean, no source changes needed for the toolchain bump (only one pre-existing warning: `Bundle.get` deprecation in `NotificationDumper.kt`). Also folds in the v0.13.8-era `ProfileScreenPreview` (was uncommitted on main — never made the v0.13.8 push) with three @Preview variants (empty / large font / compact width).
