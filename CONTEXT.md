...
- Build now exposes versionName (currently `0.8.1`) via `BuildConfig`; the Home greeting renders a small `v0.8.1-debug` pill top-right and Settings → About reflects the same value. The debug APK output is renamed to `rupee-{versionName}-{buildType}.apk` so the file itself carries the version
