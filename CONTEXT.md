...
- v0.14.0 (gravedigging audit — May 19 2026): the largest single-week sweep across the ingestion pipeline since v0.13.0. Five PRs landed across two days (#41, #42, #44 code + #43, #45 doc / version bump) closing every High and Medium tier item from [docs/gravedigging-2026-05-18.md](docs/gravedigging-2026-05-18.md). Schema went from v8 → v11 with three migrations. The originally-reported "Inbox husk" bug (pending inbox rows rendering with empty merchant + "—" amount once the recent-candidates window had rotated past them) was the seed; the audit cascaded outward to find every same-shape correctness gap. **(A) Inbox husk fix + H2 transactions list**: `HomeViewModel` was doing in-memory joins over two parallel windowed Room flows (`observePendingInboxItems(LIMIT 20)` + `observeRecentTransactionCandidates(LIMIT 20)`); when the candidate window rotated past an inbox row's reference (every marketing/promo notification was writing a candidate via the gate-rejected path), the join returned null and the UI rendered a husk. New `InboxItemWithCandidate` POJO uses `@Embedded inbox` + `@Relation candidate` so Room joins at the DB; husks structurally impossible. Same shape on the transactions list — swapped `LIMIT 20 ORDER BY occurredAt DESC` for a 30-day windowed query so back-dated manual entries stay visible. **(B) H1 + REFUND routing**: PhonePe/Paytm parsers tagged refund bodies as `ParsedTransactionKind.REFUND`, but every downstream `when` block defaulted everything-not-INCOME to EXPENSE. A ₹500 Swiggy refund was being written as a second ₹500 expense, inflating monthly spend instead of cancelling the outflow. Added explicit REFUND → INCOME branches in both parsers and lifted the canonical-type mapping into `NotificationSignalNormalizer.canonicalTypeFor(kind)`, an expression-position exhaustive `when` so a future enum addition fails compilation. New `CanonicalTypeMappingTest` pins one case per enum value. **(C) H3 storage hygiene**: `writeGateRejectedSignal` no longer writes a candidate row per gate-rejected notification (was the biggest source of recent-candidates-window churn; ~18k saved rows/year on a notification-heavy phone). Added `RawCaptureEventDao.deleteOlderThan(cutoffIso)` + repository wrapper `pruneStaleIngestionRows(now, retentionDays = 90, force)`. Dispatched fire-and-forget from `RupeeApplication.onCreate` on an app-scoped `SupervisorJob`, debounced to once per 24h via the new `app_state` K/V table (see D). **(D) H4 foreign keys + cascade deletes**: the schema had clearly-shaped parent/child relationships (raw → parsed → candidate → inbox, plus back-references to canonical) but they were convention-only. Migration `MIGRATION_8_9` rebuilt three tables (SQLite has no ADD CONSTRAINT) with explicit `@ForeignKey` declarations: CASCADE on the parent chain so pruning sweeps the children; SET NULL on back-references to `canonical_transactions` so user-visible txns survive their inbox/candidate audit rows being deleted. `PRAGMA foreign_keys = OFF` brackets the rebuild; orphan-row pre-cleanup runs first so existing data doesn't violate the new constraints. **(E) M1 magic-string removal**: `DumpOutcomeDao.mergedIntoExistingTxnId` used to derive merge-vs-fresh state via a CASE expression comparing `inbox.linkedCanonicalTransactionId` against a synthesised `'txn-' || tc.id` string — load-bearing magic that would silently break if the id format ever changed. Added explicit `inbox_items.mergedFromExistingCanonicalId` column (`MIGRATION_9_10`, plain ADD COLUMN), set by `confirmInboxItemMergedWith`. DAO selects the column directly now. **(F) M2 gate vocab**: added `reversed`, `reversal`, `chargeback`, `refund of`, `SIP installment`, `NACH mandate`, `ECS debit`, `standing instruction` to `TransactionalGate.POSITIVE_VERBS`. Reversal / chargeback / refund-of variants were silently being dropped as no-verb; SIP/NACH/ECS auto-debits matter for real-money recurring spend. **(G) M3a ATM parser**: new `AtmNotificationParser` registered FIRST in the registry so ATM bodies (HDFC/ICICI/SBI/Axis shapes — "ATM Cash Wdl Rs.5,000 from A/c XX1234", "Cash withdrawal of Rs.10,000 via ATM") route as `CASH_WITHDRAWAL` → `CASH_ADJUSTMENT` canonical type (excluded from monthly spend totals) instead of going through `IciciNotificationParser` etc. as SPEND. HIGH confidence (0.85) when amount + maskedDigits both present, but routes to Inbox via NON_SPEND_REVIEW rather than auto-create — user verifies "you withdrew Rs.X" before it disappears from spend numbers. **(H) M3b fuel-brand normalisation**: added to `MerchantNameUtils.clean` rather than a separate parser, so every bank/UPI parser inherits the fix without losing maskedDigits + provider signals. IOC / HPCL / BPCL / Shell / Nayara / Essar tokens map to the bare brand via a padded whole-word matcher; explicit BIOCON LIMITED regression test pins that the matcher won't collide on substrings. **(I) M4 persisted debounce timestamps**: new `app_state` K/V table (`MIGRATION_10_11`, single CREATE TABLE) backs `last_recurring_refresh_ms` and `last_ingestion_prune_ms`. `LocalFinanceRepository.hydrateDebounceState()` hydrates the AtomicLongs at startup; `persistDebounceTimestamp(key, valueMs)` writes through on every set, fire-and-forget on the injected `persistScope` parameter (production passes `RupeeApplication.appScope`, tests get an isolated SupervisorJob default; dropped the earlier GlobalScope use after the PR review). **(J) M5 UPI confidence tiering**: `GenericUpiNotificationParser` used to lock confidence at 0.7 (MEDIUM → Inbox always). Tiered ladder now: amount + merchant + maskedDigits → 0.85 HIGH (auto-create); amount + merchant → 0.78 MEDIUM; amount + digits → 0.62 MEDIUM; amount only → 0.5 LOW. Three pin tests with `assertEquals(confidence, 0.0001)`. **(K) M6 share-path exceptions**: three `runCatching` blocks in `DebugViewModel.shareNotificationDumps` (dump copy, outcomes snapshot build, share intent dispatch) now log to `Log.w("RupeeShare", ..., t)` and surface the exception class in the toast via a `shortReason(t)` helper. **(L) M7 single-card fallback guard**: `findMatchingCard` refused to fall back to the only active card when the parser captured maskedDigits or a sourceCardHint that didn't match — was silently overwriting the wrong card's statement due date when a forwarded SMS or household-card alert leaked through. **(M) L3 CrashReporter.readLog → Result<String>**: distinguishes "no crashes yet" from "couldn't read the file"; debug screen shows the exception class on read failure. Two trivial migrations (ADD COLUMN, CREATE TABLE) and one table-rebuild migration (FKs) landed across the three code PRs. Build: assembleDebug + assembleRelease both clean; 216 unit tests, 0 failures (up from ~140 at the start of the audit).
- v0.15.0-alpha.2 (alpha hotfix — cascade-delete bug, May 23 2026): the very first ALPHA-STAGE-ROLL surfaced an alpha-blocker. User installed alpha.1, captured a dump after a real Kotak credit notification, and reported "nothing has come in the inbox" — even though the dump line said `kind=ingested` with `inboxItemId` populated. Deep-research traced this to a **SQLite REPLACE + ON DELETE CASCADE interaction** that has been silently destroying every INBOX_PENDING-bound inbox row since v0.14.1. **Mechanism:** `TransactionCandidateDao.upsertTransactionCandidate` used `@Insert(onConflict = REPLACE)`, which Room renders as `INSERT OR REPLACE`. SQLite REPLACE deletes the conflicting row first, then inserts. Because `inbox_items.transactionCandidateId` has `onDelete = CASCADE`, every REPLACE on a candidate row cascade-deleted any inbox row pointing at it. `NotificationSignalNormalizer.normalizeLocked` does three writes — insert candidate → insert inbox → re-upsert candidate to backfill `linkedInboxItemId` — and the third write's REPLACE silently destroys the second write's inbox row. The `IngestionResult.Ingested` still carries the now-orphaned inboxItemId, which is why dumps showed `kind=ingested` while the live DB showed `inboxItemId=NULL` on every INBOX_PENDING candidate. **Bug archaeology:** confirmed by replaying past production dumps — v0.14.0 had a different bug (H4 FK violation eating bodies) that masked this one; v0.14.2 (3/3), v0.14.5 (carried forward), and v0.15.0-alpha.1 (2/2) all show the signature. **Fix:** switch the DAO from `@Insert(onConflict = REPLACE)` to `@Upsert` (Room 2.5+; we're on 2.7.1). `@Upsert` generates `INSERT ... ON CONFLICT(id) DO UPDATE SET ...` — in-place update, no delete, no cascade. **Verification:** reproduced the bug with a new instrumented test on a real Nothing-A015 device (the test fails at `inbox_items.size == 1` pre-fix, passes post-fix). The existing IngestionPipelineTest had been silently failing since Sprint 2's S1.3 promotions because its CRED body now lands AUTO_CREATED instead of INBOX_PENDING; updated to use a Kotak debit body which stays MEDIUM by design (no merchant). **Three pre-existing test failures filed to Sprint 4:** MigrationTest schema-asset packaging (3 tests), DumpOutcomeDaoTest FK violations (2 tests), CANONICAL-AUDIT-TRAIL (the same REPLACE+SET_NULL pattern on `CanonicalTransactionDao` — softer bug than cascade-delete, separate hotfix candidate). **What's NOT recovered:** inbox rows already lost between v0.14.1 and v0.15.0-alpha.1 are unrecoverable — candidate rows still exist as orphaned INBOX_PENDING with stale `linkedInboxItemId` pointers. v0.15.0-alpha.2 only fixes new ingestion going forward. Build: connectedDebugAndroidTest passes for IngestionPipelineTest (3 cases), full unit suite + debug + release APKs green.
- v0.15.0-alpha.1 (Sprint 3 — **alpha cut**, May 23 2026): the first build judged stable enough for a small closed-tester group. Three items closed across three PRs (#75 BABYPROOF-INPUTS, #76 DUMP-REPLAY-REBASELINE, this release PR). **(1) BABYPROOF-INPUTS**: new `ui/Inputs.kt` with three reusable composables — `CurrencyInputField` (₹ prefix, digit-only filter, Indian-grouping `visualTransformation` so `100000` reads as `1,00,000` while typing), `DateField` (read-only OutlinedTextField that opens a Material 3 `DatePickerDialog` — replaces the EMI/card "type yyyy-MM-dd into a text box" sites), `ProviderDropdown` (typeable dropdown over the bank/card providers our parsers cover). Sweep replaced every offending free-text input across Cards & EMIs (EMI monthly amount, tenure capped at 120, next-due date; card amount-due, due-date), Budgets (monthly + per-category), Onboarding (monthly budget, bank/card provider, starting cash), Inbox review amount draft, Manual entry amount. 14 pin tests on the sanitiser + Indian-grouping formatter + ISO date validator. **AccountPicker intentionally deferred** because it needs a schema change on `canonical_transactions` + repo signature change. **(2) DUMP-REPLAY-REBASELINE**: tightened the `DumpReplayHarness` regression tolerance from 5 → 1 and added a separate `autoCreated` baseline so the S1.3 tier-promotion work has its own regression alarm. v0.13.3 dump now baselines at 32 accepted / 6 AUTO_CREATED (was loose at 32 / ?). v0.14.0 dump at 14 accepted / 5 AUTO_CREATED (was loose at 14 / ?). **(3) ALPHA-CUT**: bumped `versionCode 41 → 42`, `versionName 0.14.5 → 0.15.0-alpha.1`. New `CHANGELOG.md` at the repo root (didn't exist before; this is the first entry with full history back to v0.14.3). Built signed release APK + debug APK. Git-tagged `v0.15.0-alpha.1`. **ALPHA-STAGE-ROLL** ran in parallel — see `docs/alpha-stage-roll-log.md` for the structured soak log. Build: assembleDebug + assembleRelease + 250+ unit tests + dump-replay baseline all green.
- v0.14.5 (Sprint 2 — polish on the existing theme, May 22 2026): six items closed across six PRs (#65 backlog rescope, #66 INBOX-WHY-COPY, #67 CAL-INTER, #68 COLDSTART, #69 S1.3-rest, #70 MANUAL-FAST, #71 POLISH-1). All flavor-agnostic — the Aviate-vs-vwfndr design call moved to post-1.0 as `REVAMP` in the backlog. **(1) INBOX-WHY-COPY**: every Inbox row's reason chip now reads as user-facing copy (`AMOUNT ONLY`, `NO MERCHANT`, `DUPLICATE SUSPECTED`, `MERCHANT UNCLEAR`, etc.) instead of the enum echo; pure helper at `home/InboxReasonCopy.kt` with 9 pin tests. **(2) CAL-INTER**: horizontal-drag gesture on the Calendar's weekday header + grid fires `onPrev`/`onNext` past a 60dp threshold; chevrons + bottom-sheet drag-dismiss both still work. **(3) COLDSTART**: theme-native skeleton primitives at `ui/Skeletons.kt` (surfaceVariant pills + 1.8s alpha pulse) replace the empty-state copy that flashed during the brief `isSeeding == true` window on Home/Inbox/Transactions; empty-state copy rewritten as invitations. **(4) S1.3 (rest)**: the remaining 8 parsers (Atm/Cred/Emi/GPay/Generic/Icici/Kotak/Paytm/PhonePe) migrated off hardcoded confidence ladders onto `EvidenceTally`. Per-parser weights tuned to preserve tier landings across the dump-replay corpus with three deliberate within-tier promotions (ICICI SPEND with amount+merchant, ICICI INCOME with amount+digits, ATM with location) that bring per-parser scoring into shape-symmetry with the CRED analogues. Each parser version-bumps. **(5) MANUAL-FAST**: AssistChip row on the manual-entry sheet shows the 5 most recent unique-merchant EXPENSE transactions; tap fills merchant + category + mode in one gesture. EXPENSE-only because INCOME entries are too varied for "similar to last" suggestions. **(6) POLISH-1**: content-card corner radius standardised at 24.dp across Home + Settings (WeeklySpendCard 20→24, SettingsCard 20→24; HeroBudgetCard keeps 28 for hero emphasis, ReviewRowCard keeps 18 for row idiom). The static "Spent" decoration on WeeklySpendCard becomes a primary-tinted income pill ("Received" / ₹X) when monthly income > 0, folding in the prior duplicated body line. Build: assembleDebug + 250+ unit tests + dump-replay baseline all green.
- v0.13.10 (app-wide lint cleanup, May 14 2026): two PRs landed back-to-back. **(A) Gemini-in-AS pass on MainActivity.kt** — promoted fully-qualified `com.zegrt.rupee.*` refs to top-level imports (`CardsEmisViewModel`, `BudgetsScreen`, `CalendarUiState`, etc.), `Uri.parse` → `.toUri()` via `androidx.core.net.toUri`, `BuildConfig` import dropped (same-package, redundant), `@OptIn(ExperimentalMaterial3Api::class)` added to `CategoryChipRow` (previously implicit via parent file scope), inlined `SetupToggleCard` parameters (only one caller — YAGNI), removed unused `HomeTabChip`. Quirk: Gemini "fixed" Compose state-write linter warnings by adding `if (showX) showX = false` guards across 9 dialog dismiss / close-button sites — tautologically true since the dialog only renders when `showX == true`, but harmless dead code; kept for now. **(B) Claude audit applied the same hygiene rules to the rest of the codebase** — driven by `./gradlew lintDebug` + manual grep. Real bugs fixed: 2× `MissingPermission` in `BudgetAlertManager` + `DuesAlertManager` — both `NotificationManagerCompat.notify()` calls now gate on `areNotificationsEnabled()` **before** the dedupe-pref write so a user who re-grants POST_NOTIFICATIONS later still gets the next alert. 6× `UseKtx` (SharedPreferences `.edit().apply()` → `prefs.edit { ... }` ×4, `Uri.parse` → `.toUri()` ×1, the last in `DebugViewModel`). 2× `MonochromeLauncherIcon` — added `<monochrome>` tag (reusing `ic_launcher_foreground`) to both adaptive-icon XMLs for Android 13+ themed icons. 24 fully-qualified ref strips across 8 files. Skipped: `ObsoleteSdkInt` for `mipmap-anydpi-v26` — AAPT can't resolve adaptive-icon resources from `mipmap-anydpi` without the version qualifier; left v26. Remaining GradleDependency / NewerVersionAvailable lint warnings are on appcompat / activity / lifecycle which are on current versions for their lines. `OldTargetApi` (35 → 36) deferred — needs Android 16 testing.
