...
- Build now exposes versionName (currently `0.12.0`) via `BuildConfig`; the Home greeting renders a small `v0.12.0-debug` pill top-right and Settings → About reflects the same value. The debug APK output is renamed to `rupee-{versionName}-{buildType}.apk` so the file itself carries the version
